...
MODEL_PATH = os.path.join(BASE_PATH, 'modelo_catboost_comparativo.cbm')
...
def predict_catboost_image():
    ...
    preds_flat = model.predict(X_flat).flatten()
    ...
    plt.title("Resultado Clasificacion CatBoost (Pixel-Based)")
    plt.axis('off')
    plt.tight_layout()
    plt.show()

    save_path = os.path.join(BASE_PATH, 'resultado_catboost.tif')
